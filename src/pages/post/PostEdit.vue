...
onMounted(async () => {
  try {
    const { data } = await api.get(`/api/boards/${categoryId.value}/posts/${postId.value}`)
    setValues({ title: data.data.title, content: data.data.content })
  } catch (error) {
...
const onSubmit = handleSubmit(async (values) => {
  try {
    await api.put(`/api/boards/${categoryId.value}/posts/${postId.value}`, {
      ...values,
      userId: authStore.user.userId,
    })
    toast.success('수정되었습니다.')
    router.push(`/boards/${categoryId.value}/posts/${postId.value}`)
  } catch (error) {
    ...
    <form class="space-y-6" @submit="onSubmit">
      ...
      <div class="flex w-full justify-end space-x-2">
        ...
        <Button
          type="button"
          variant="outline"
          @click="router.push(`/boards/${categoryId}/posts/${postId}`)"
        >
          취소
        </Button>
      </div>
